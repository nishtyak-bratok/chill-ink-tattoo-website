# PowerShell script to rollback all master cards to original layout
Write-Host "🔄 Rolling back all master cards to original layout..." -ForegroundColor Yellow

# Read the current index.html file
$content = Get-Content "index.html" -Raw

# Function to rollback a card to original layout
function Rollback-MasterCard {
    param(
        [string]$MasterId,
        [string]$MasterName,
        [string]$BorderColor,
                ...
                <div class="bg-white p-4 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border-2 border-$BorderColor relative overflow-hidden cursor-pointer" onclick="openMasterGallery('$MasterId')">
                    ...
                    <div class="grid grid-cols-2 gap-2 mb-4" style="height: 120px;">
                        <div class="rounded-lg overflow-hidden cursor-pointer hover:opacity-80 transition-opacity" onclick="event.stopPropagation(); openMasterGallery('$MasterId'); setTimeout(() => highlightWorkInGallery('$MasterId', 1), 500);">
                            <img src="gallery/$MasterId/$MasterId-work-1.jpg" alt="Работа 1" class="w-full h-full object-cover" loading="lazy">
                        </div>
                        <div class="rounded-lg overflow-hidden cursor-pointer hover:opacity-80 transition-opacity" onclick="event.stopPropagation(); openMasterGallery('$MasterId'); setTimeout(() => highlightWorkInGallery('$MasterId', 2), 500);">
                            <img src="gallery/$MasterId/$MasterId-work-2.jpg" alt="Работа 2" class="w-full h-full object-cover" loading="lazy">
                        </div>
                        <div class="rounded-lg overflow-hidden cursor-pointer hover:opacity-80 transition-opacity" onclick="event.stopPropagation(); openMasterGallery('$MasterId'); setTimeout(() => highlightWorkInGallery('$MasterId', 3), 500);">
                            <img src="gallery/$MasterId/$MasterId-work-3.jpg" alt="Работа 3" class="w-full h-full object-cover" loading="lazy">
                        </div>
...
"@
    ...
    return $content -replace [regex]::Escape($compactPattern), $originalPattern
}

# Rollback each master card
Write-Host "Rolling back Булат's card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "rocket" -MasterName "Булат" -BorderColor "orange-500" -GradientColor "orange-500 to-purple-500" -TextColor "orange-600" -Specialization "Графика" -Experience "10+ лет" -Status "🏛️ Резидент студии"

Write-Host "Rolling back Арсений's card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "lozhkin" -MasterName "Арсений" -BorderColor "red-500" -GradientColor "red-500 to-pink-500" -TextColor "red-600" -Specialization "Японская татуировка" -Experience "8+ лет" -Status "🏛️ Резидент студии"

Write-Host "Rolling back Анастасия's card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "nastasia" -MasterName "Анастасия" -BorderColor "pink-500" -GradientColor "pink-500 to-red-500" -TextColor "pink-600" -Specialization "Реализм, Чикано" -Experience "10+ лет" -Status "🏛️ Резидент студии"

Write-Host "Rolling back Дина's card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "dina" -MasterName "Дина" -BorderColor "teal-500" -GradientColor "teal-500 to-cyan-500" -TextColor "teal-600" -Specialization "Акварельная, Ботаническая" -Experience "8 лет" -Status "🏛️ Резидент студии"

Write-Host "Rolling back Михаил Б.'s card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "barabanov" -MasterName "Михаил Б." -BorderColor "indigo-500" -GradientColor "indigo-500 to-purple-500" -TextColor "indigo-600" -Specialization "Neotraditional" -Experience "10+ лет" -Status "🌍 Гостевой мастер"

Write-Host "Rolling back Фларик's card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "flo" -MasterName "Фларик" -BorderColor "emerald-500" -GradientColor "emerald-500 to-teal-500" -TextColor "emerald-600" -Specialization "Орнаментал, Полинезия, Борнео" -Experience "10+ лет" -Status "🏛️ Резидент студии"

Write-Host "Rolling back Михаил А.'s card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "atlasov" -MasterName "Михаил А." -BorderColor "gray-500" -GradientColor "gray-500 to-slate-500" -TextColor "gray-600" -Specialization "Минималистичная татуировка" -Experience "10+ лет" -Status "🏛️ Резидент студии"

Write-Host "Rolling back Лейла's card..." -ForegroundColor Yellow
$content = Rollback-MasterCard -MasterId "leyla" -MasterName "Лейла" -BorderColor "indigo-500" -GradientColor "indigo-500 to-purple-500" -TextColor "indigo-600" -Specialization "Орнаментальная татуировка" -Experience "5 лет" -Status "🏛️ Резидент студии"

# Save the rolled back content
$content | Out-File "index.html" -Encoding UTF8

Write-Host "✅ All master cards rolled back to original layout!" -ForegroundColor Green
Write-Host "📋 Cards restored to:" -ForegroundColor Cyan
Write-Host "  - Original padding (p-8)" -ForegroundColor White
Write-Host "  - Original avatar size (w-32 h-32)" -ForegroundColor White
Write-Host "  - Center-aligned layout" -ForegroundColor White
Write-Host "  - Original text sizes" -ForegroundColor White
Write-Host "  - Original aspect-square previews" -ForegroundColor White
